...
namespace pr{
    ...
    cv::Mat PlateSegmentation::classifyResponse(const cv::Mat &cropped){//分类
        cv::Mat inputBlob = cv::dnn::blobFromImage(cropped, 1/255.0, cv::Size(22,22), cv::Scalar(0,0,0),false);
        net.setInput(inputBlob,"data");
        return net.forward();
    }

    void drawHist(float* seq,int size,const char* name){//绘制图像的直方图
    ...
    cv::Rect boxFromCenter(const cv::Point center,int left,int right,int top,int bottom,cv::Size bdSize)//找中心点
    {
    ...
    cv::Rect boxPadding(cv::Rect rect,int left,int right,int top,int bottom,cv::Size bdSize)//找车牌边缘框
    {
...
//            cv::waitKey(0);
    ...
    void avgfilter(float *angle_list,int size,int windowsSize) {//均值滤波
